...
function render_society_news_template($is_minimazed = false)
{
?>
	<?php render_topic_bar("Общество", true, array(
		'link' => get_site_url() . '/news-list/obshhestvo/',
	...
	<?php $query_pallete = get_posts(array(
		'post_status' => 'publish',
		'posts_per_page' => -1,
		'post_type' => 'news',
		'post_count' => 7,
	...
	<div class="box-column-gap society-news" style="margin-bottom: 30px;">
		<?php if ($is_minimazed) : ?>
			<div class="box-line-gap">
				<div class="box">
					<?php render_new_template_image($query_pallete[0]->ID); ?>
				</div>
				<div class="box box-list">
					<?php render_news_template_line($query_pallete[1]->ID, false, true); ?>
					<?php render_news_template_line($query_pallete[2]->ID, false, true); ?>
				</div>
			</div>
		<?php else : ?>
			...
			<div class="box">
				<?php render_news_template_line($query_pallete[6]->ID, true, false); ?>
			</div>
			<div class="box-line-gap">
				<div class="box">
					<?php render_new_template_image($query_pallete[3]->ID); ?>
				</div>
				<div class="box box-list">
					<?php render_news_template_line($query_pallete[4]->ID, false, true); ?>
					<?php render_news_template_line($query_pallete[5]->ID, false, true); ?>
				</div>
			</div>
		<?php endif; ?>
	</div>
